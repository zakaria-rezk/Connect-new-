<template>
  <div class="container container-fluid position-relative">
    <Error
      ...
      v-if="Modal"
      :link="false"
      title="تم طلب الخدمة برجاء انتظار الرد"
    />
    ...
    >
      ...
      <div class="button-wrapper">
        ...
        <button
          @click.prevent="
            sendRequest(service.name, service.price, service.description)
          "
          class="btn fill"
    ...
    <div
      v-if="useOfferedServices.G_offeredServices.length === 0"
      class="mx-5 text-warning"
    >
      <h1>لا يوجد خدمات ف الوقت الحالي</h1>
    </div>
...
const props = defineProps(["BussinsID", "admin"]);
...
const nextPage = async () => {
  ...
  setTimeout(async () => {
    await useOfferedServices.getOfferdServices(
      pageIndex.value,
      route.params.id
    );
    isAnimted.value = !isAnimted.value;
